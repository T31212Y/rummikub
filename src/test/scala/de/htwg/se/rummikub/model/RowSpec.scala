...
class RowSpec extends AnyWordSpec {
  "Row" should {
    ...
    "change string list to token list" in {
      ...
      val row = Row(tokens)
      Row.changeStringListToTokenList(tokenStrings, tokenFactory) should be (List(tokenFactory.createNumToken(1, Color.RED), tokenFactory.createNumToken(2, Color.BLUE), tokenFactory.createNumToken(3, Color.GREEN), tokenFactory.createNumToken(4, Color.BLACK)))
    }
  }
}
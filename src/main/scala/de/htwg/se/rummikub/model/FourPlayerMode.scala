...
case class FourPlayerMode(playerNames: List[String]) extends GameModeTemplate {
    ...
    override def createPlayingField(players: List[Player]): PlayingField = {
        ...
        val innerField = new Table(cntRows - 4, boardP13.size(boardP13.wrapBoardRowSingle(boardP13.boardELRP12_1)) - 2)

        PlayingField(players, List(boardP13, boardP24), innerField)
    }

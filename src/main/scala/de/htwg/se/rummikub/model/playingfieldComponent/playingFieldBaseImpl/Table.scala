...
case class Table(cntRows: Int, length: Int, tokensOnTable: List[List[TokenStructureInterface]] = List()) {
    ...
    def add (e: List[TokenStructureInterface]): Table = this.copy(tokensOnTable = this.tokensOnTable :+ e)

    def remove(tokensToRemove: List[TokenStructureInterface]): Table = {
    ...
    def getGroup(index: Int): Option[List[TokenStructureInterface]] = {
        tokensOnTable.lift(index)
    }

    def updateTokensOnTable(index: Int, newTokens: List[TokenStructureInterface]): Table = {
        val updated =

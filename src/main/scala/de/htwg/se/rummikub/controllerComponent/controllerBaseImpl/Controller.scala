...
class Controller(var gameMode: GameModeTemplate) extends ControllerInterface {
    ...
    def addRowToTable(row: TokenStructureInterface, currentPlayer: PlayerInterface): (List[TokenInterface], PlayerInterface) = {
        ...
        if (unmatched.nonEmpty) {
            ...
            val updatedTokensStructures: List[TokenStructureInterface] = remainingTokensFlat.map(token => tokenStructureFactory.createSingleTokenStructure(token))
    ...
    def getState: GameState = playingField match {
        ...
        case None => GameState(
                        table = null, // oder ein Dummy-TableInterface
                        players = Vector.empty,
                        boards = Vector.empty,
                        currentPlayerIndex = 0,
                        stack = null // oder ein Dummy-TokenStackInterface
                    )
    }
